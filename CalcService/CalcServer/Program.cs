...
{
    builder.AddService<CalcService>((serviceOptions) =>
    {
        // Set the default host name:port in generated WSDL and the base path for the address 
        serviceOptions.BaseAddresses.Add(new Uri($"http://{HOST_IN_WSDL}/CalcService"));
        serviceOptions.BaseAddresses.Add(new Uri($"https://{HOST_IN_WSDL}/CalcService"));
    })
    .AddServiceEndpoint<CalcService, ICalcService>(new BasicHttpBinding(), "/")
    .AddServiceEndpoint<CalcService, ICalcService>(new BasicHttpBinding(BasicHttpSecurityMode.Transport), "/");
});
